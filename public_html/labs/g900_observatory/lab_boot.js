...
import { loadG900Phase30 } from "./kernel/g900_data.js";
import { buildVertices, buildEdges } from "./kernel/g900_geometry.js";
...
function saveCurrentLensToLocalStorage() {
  const fallback = currentPresetLabel() || "G900 lens";
...
function sourceLabel() {
  ...
  if (String(data.source).includes("g900_phase30_combined_graph_export")) {
...
function draw() {
  ...
  const surfaceScene = {
    candidate,
    view: "external_g900",
    vertices: buildVertices(candidate, "external_g900", tick, playing),
    edges: buildEdges(candidate, "external_g900"),
  };
...
async function main() {
  ...
  data = await loadG900Phase30();
  candidate = data.candidates.find((c) => c.id === PREFERRED_CANDIDATE_ID) || data.candidates[0];

  renderLocalLensOptions("");
  applySettingsFromUrl();

  if (!candidate) throw new Error("No G900 candidate loaded");
