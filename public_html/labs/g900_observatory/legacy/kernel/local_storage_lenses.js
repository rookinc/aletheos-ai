const G900_LENS_STORAGE_KEY = "g900_saved_lenses_v1";
const G900_LENS_SCHEMA_VERSION = 1;
...
function normalizeLens(raw) {
  ...
  return {
    ...
    schemaVersion: Number(raw.schemaVersion || G900_LENS_SCHEMA_VERSION),
  };
}

function readRawStorage(storage) {
  try {
    return storage.getItem(G900_LENS_STORAGE_KEY);
  } catch (err) {
    return null;
  }
}
...
function writeRawStorage(storage, value) {
  try {
    storage.setItem(G900_LENS_STORAGE_KEY, value);
    return true;
  } catch (err) {
    return false;
  }
...
export function localLensStorageAvailable(storage = window.localStorage) {
  ...
  const testKey = G900_LENS_STORAGE_KEY + "_test";
  try {
...
export function createLocalLens({ name, params }, storage = window.localStorage) {
  ...
  const lens = {
    ...
    createdAt: now,
    updatedAt: now,
    schemaVersion: G900_LENS_SCHEMA_VERSION,
  };
...
export function clearLocalLenses(storage = window.localStorage) {
  try {
    storage.removeItem(G900_LENS_STORAGE_KEY);
    return true;
  } catch (err) {
    return false;
  }
